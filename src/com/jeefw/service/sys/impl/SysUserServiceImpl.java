...
public class SysUserServiceImpl extends BaseService<SysUser> implements SysUserService {
	...
	// 1.获取用户信息（将数据库查询出来的信息再处理，增加字段的中文意思）
	...
	// 2.获取个人资料信息（将数据库查询出来的信息再处理，增加头像）
	public SysUser getSysUserWithAvatar(SysUser sysuser) {
        ...
        // 根据 type，typeid获取 attachment 对象
		Attachment attachment = attachmentDao.getByProerties(new String[] { "type", "typeId" }, new Object[] { (short) 1, sysuser.getId() });
		if (null != attachment) {
			entity.setFilePath(attachment.getFilePath()); //如果不为空,就设置sysUser的filepath
		} else {
			entity.setFilePath("/static/assets/avatars/profile-pic.jpg"); //如果为空,就设置sysuser的filepath为默认值。
		}
		return entity;
	}
    // 3.更新用户信息，密码
