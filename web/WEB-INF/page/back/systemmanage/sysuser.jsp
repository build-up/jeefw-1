        ...
        $('.page-content-area').ace_ajax('loadScripts', scripts, function() {
        	...
        	jQuery(function($) {
        		...
        		jQuery(grid_selector).jqGrid({
        			...
        			url : "${contextPath}/sys/sysuser/getSysUser", //获取数据的地址。
        			datatype : "json",  //从服务器端返回的数据类型，默认xml。可以设置json
        			height : 450,  //表格高度
        			colNames : ['', 'ID', '姓名', '性别', '邮箱', '联系电话', '生日', '所属部门', '角色', '是否禁用', '最后登录时间'],//列显示名称，是一个数组对象
        			colModel : [ {   //常用到的属性
        				name : '',  //name列显示名称
        				index : '',  //index传到服务器端用来排序用的列名
        				width : 80,   //列宽度
        				fixed : true, //列宽度是否要固定不可变
        				sortable : false,  //是否可以排序
        				resize : false,  //是否重新设置大小
        				formatter : 'actions', //对列进行格式化时设置的函数名或者类型
        				formatoptions : {  //对于form进行编辑时的属性设置
        					keys : true,
        				...
        				label : 'ID',
        				width : 60,
        				sorttype : "long", //排序的数据类型 string text
        				search : false  //该字段是否允许搜索(查询)
        			}, {
        				name : 'userName',
        				index : 'userName',
        				label : '姓名',
        				...
        				editoptions : {size : "20", maxlength : "50"}, //针对edittype配置的控件类型的属性设置
        				searchoptions : {sopt : ['cn']}, //配置搜索选项
        				editrules : {required : true} //编辑的规则,且为数字类型,并且为必输字段。
        			}, {
        				name : 'sexCn',
        				index : 'sex',
        				label : '性别',
        				width : 80,
        				editable : true,
        				edittype : "select",//该字段控件类型(text,textarea,select,checkbox,password,button,image ,file)string text
        				...
        				search : false,
        				sorttype : 'date',
        				unformat : pickDate  //pickDate是一个函数
        			}, {
        			...
        			editurl : "${contextPath}/sys/sysuser/operateSysUser" //定义对form编辑时的url
        			//caption : "用户管理列表",
        			//autowidth : true,
        			/**
        			grouping : true, 
        		...
        		jQuery(grid_selector).jqGrid('navGrid', pager_selector, { // navbar options
        			...
        			editicon : 'ace-icon fa fa-pencil blue', //编辑图标
        			...
        			refresh : true,
        			refreshicon : 'ace-icon fa fa-refresh blue',  //刷新图标
