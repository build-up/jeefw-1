...
<script type="text/javascript">
        ...
        $('.page-content-area').ace_ajax('loadScripts', scripts, function() {
        	...
        	jQuery(function($) {
                /*grid表格*/
                var grid_selector = "#grid-table"; <%--一个table用户显示数据--%>
        		var pager_selector = "#grid-pager"; <%--一个定义翻页用的导航条--%>

        		// resize to fit page size 调整grid以适合页面大小
        		$(window).on('resize.jqGrid', function() {
        			$(grid_selector).jqGrid('setGridWidth', $(".page-content").width());
        		})
        		// resize on sidebar collapse/expand  调整grid和滚动条
        		...
        		jQuery(grid_selector).jqGrid({
        			...
        			colModel : [ {
        				...
        				search : false,
        				sortable : false,
        				formatter : authorityFormatter  //是一个函数，当点击授权的时候会弹出一个模态框。
        			} ],
                ...
                /*模态框的设置,点击授权的时候,会弹出一个模态框*/
        		function authorityFormatter(cellvalue, options, cell) {
        			var template = "<button data-toggle='modal' onclick='javascript:$(\"#modal-table\").modal(\"toggle\");" +
                            "$(\"#roleKeyId\").val(\"" + cell.roleKey +"\")' class='btn btn-xs btn-warning'>" +
                            "<i class='ace-icon fa fa-flag bigger-120'></i>" +   //一个图标
                            "</button>";
        			return template;
        		}
        		var treeflag = 0;
        		...
        		jQuery(grid_selector).jqGrid('navGrid', pager_selector, { // navbar options
        			edit : <shiro:hasPermission name="${ROLE_KEY}:role:edit">true</shiro:hasPermission><shiro:lacksPermission name="${ROLE_KEY}:role:edit">false</shiro:lacksPermission>,
        			editicon : 'ace-icon fa fa-pencil blue',
        			add : <shiro:hasPermission name="${ROLE_KEY}:role:add">true</shiro:hasPermission><shiro:lacksPermission name="${ROLE_KEY}:role:add">false</shiro:lacksPermission>,
        			addicon : 'ace-icon fa fa-plus-circle purple',
        			del : <shiro:hasPermission name="${ROLE_KEY}:role:delete">true</shiro:hasPermission><shiro:lacksPermission name="${ROLE_KEY}:role:delete">false</shiro:lacksPermission>,
        			delicon : 'ace-icon fa fa-trash-o red',
        			search : <shiro:hasPermission name="${ROLE_KEY}:role:search">true</shiro:hasPermission><shiro:lacksPermission name="${ROLE_KEY}:role:search">false</shiro:lacksPermission>,
        			searchicon : 'ace-icon fa fa-search orange',
        			refresh : true,
        			refreshicon : 'ace-icon fa fa-refresh blue',
        			view : <shiro:hasPermission name="${ROLE_KEY}:role:view">true</shiro:hasPermission><shiro:lacksPermission name="${ROLE_KEY}:role:view">false</shiro:lacksPermission>,
        		...
        		if(<shiro:hasPermission name="${ROLE_KEY}:role:export">true</shiro:hasPermission><shiro:lacksPermission name="${ROLE_KEY}:role:export">false</shiro:lacksPermission>){
